...
def get_model():
    model = joblib.load("pipeline/model.joblib")
    return model
...
column_Tugba.image('galeri/tugba_pp.png', width=300)
...
column_Temel.image('galeri/temelinko.png', width=300)
...
column_left.image('galeri/DataCheck.jpeg', width=454)
column_left.markdown("* check_df(data) Fonksiyonu Oluşan Çıktılar: Shape, Null Analizi, Veri Türü")

column_middle.image('galeri/myplot.jpeg', use_column_width=True)
column_middle.markdown("* BoxPlot Grafiği")

column_right.image('galeri/myplot_corr.jpeg', use_column_width=True)
...
column_left1.image('galeri/accuracy_feoncesi.jpeg', use_column_width=True)
column_left1.markdown("* Accuracy Değeri Öznitelik Çıkarımı Öncesi")

column_right1.image('galeri/accuracy_fesonrasi.jpeg', use_column_width=True)
column_right1.markdown("* Accuracy Değeri Öznitelik Çıkarımı Sonrası")

column_left1.image('galeri/f1_feoncesi.jpeg', use_column_width=True)
column_left1.markdown("* F1 Değeri Öznitelik Çıkarımı Öncesi")

column_right1.image('galeri/f1_fesonrasi.jpeg', use_column_width=True)
...
column_left2.image('galeri/RFsonuc.jpeg', width=780)
column_left2.markdown("* RF (Random Forest) Model Sonuçları")

column_right2.image('galeri/ConfusionMatrix.jpeg', width=780)
...
if column_model.button("Tahmin Etmek İçin Basınız"):
    ...
    if prediction == 1:
        column_modelresult.image('galeri/diyabetsiniz.png', width=600)
    else:
        column_modelresult.image('galeri/diyabetdegilsiniz.png', width=600)
